...
var SearchModel = function (id) {
    ...
    self.EditInvoiceDetails = function (item) {
        ...
        $("#hfcygid").val(item.invoice_Cygnet_Gid);
        $("#hfNonSupplierId").val(item.invoice_supplier_gid);
            ...
            success: function (response) {
                ...
                if (Data1.length > 0) {
                    ...
                    if ($.trim(Data1[0].SupplierCode) != "") {
                        ...
                        $("#txtVendorName").addClass("required"); $("#txtVendorName").val("");
                        //$("#txtVendorName").attr("disabled", false);
                    }
                    ...
                    if (_value == "Y") {  //vadivu changes 30-12-2020
                ...
                var Cygid = $("#hfcygid").val();
                //Ramya modified - 15 Jan 21
                if (Cygid != null && Cygid != "" && Cygid != "0") {

                    $("#txtVendorName").attr("disabled", "disabled");
                    $("#txtInvoiceDate").attr("disabled", true);
                    $("#txtServiceMonth").attr("disabled", true);
                    $("#txtInvoiceAmount").attr("disabled", true);
                    //$("#txtWithoutTaxAmount").attr("disabled", false);
                    $("#txtInvoiceDescription").attr("disabled", false);
                    $("#txtInvoiceNo").attr("disabled", true);
                    $("#txtGSTINpan").attr("disabled", true);
                    $("#txtGSTINvertical").attr("disabled", true);
                    $("#search").show();
                ...
                else if ((Cygid == null || Cygid == "" || Cygid == "0") && _value=="Y") {
                    $("#search").show();
                }
                if (response.Data2 != null && response.Data2 != "" && JSON.parse(response.Data2) != null)
                    Data2 = JSON.parse(response.Data2);
    ...
    self.SaveInvoiceDetails = function () {
        ...
        debugger;
        if ($.trim(ServiceMonth) == "" || ServiceMonth.includes("undefined")) {
    ...
    self.SubmitGSTTax = function () {
        ...
        var HSNgid = $("#ddlGSTHSNCode option:selected").val();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                */
                ...
                $("#hfcygid").val(Data13[0].invoice_Cygnet_Gid);

                var cygnid = $("#hfcygid").val(); //selva

                if (cygnid != null && cygnid != "" && cygnid != "0") { // ramya modified on 30 sep 22
                    self.ECFInvoiceArray(Data13);
                    $("#addinvoice").hide(); //selva
                    $("#rdogstChargedFlag").hide();
                }
                else {
...
$(document).ready(function () {
    ...
    $("input[name=rdogstChargedFlag]:radio").change(function () {
        ...
        if ($("input[name=rdogstChargedFlag]:radio[checked=checked]").attr("value") == "Y") {
            $("#tabGST").show(); $("#txtVendorName").attr("disabled", true);
            $("#tabRCM").hide(); //Add GSTIN split on 02-03-18
            $("#btnCygnetSearch").attr("style", "display:block;");
        }
        else {
            $("#tabGST").hide();
            $("#txtVendorName").attr("disabled", false);
            $("#tabRCM").show(); //Add GSTIN split on 02-03-18
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (Data1.length > 0) {
                ...
                $("#txtGSTINState").prop('disabled', true); //ramya added 18 Jan 21
