...
var SearchModel = function (id) {
    ...
    self.GSTChangeTaxType = function () {
      ...
      //  $("#txtGSTTaxRate").attr("disabled", true);
        ...
        $("#txtGSTTaxableAmount,#txtGSTTaxAmount").removeClass("required").removeClass("valid");
        if (Amount > 0) {
    ...
    self.AddInvoiceDetails = function () {
        $('#divInvoicePettyCash').attr("style", "display:block;");
        objDialogyInvoicePC.dialog({ title: 'Add Invoice Details', width: '1100', height: '700' });
        objDialogyInvoicePC.dialog("open");

        $("#ddlgstReceiverLoc,#ddlgstProviderLoc,#hdfSVendorId,#hdfSVendorName").val("0");
        $("#txtInvoiceDate,#txtInvoiceNo,#txtServiceMonth,#txtInvoiceAmount,#txtWithoutTaxAmount,#txtInvoiceDescription,#txtVendorName,#txtGSTINVendor,#txtGSTINFICCL").val("");
        ...
        $("#txtGSTINVendor,#txtInvoiceDate,#txtInvoiceNo,#txtServiceMonth,#txtInvoiceAmount,#txtWithoutTaxAmount,#txtInvoiceDescription,#txtVendorName,#ddlgstReceiverLoc,#ddlgstProviderLoc").removeClass("valid").removeClass("required").addClass("required");
    ...
    self.EditInvoiceDetails = function (item) {
        $('#divInvoicePettyCash').attr("style", "display:block;");
        objDialogyInvoicePC.dialog({ title: 'Invoice Details', width: '1100', height: '700' });
        objDialogyInvoicePC.dialog("open");
    ...
    self.AddGSTTax = function () {
        $('#GSTDialogPettyCash').attr("style", "display:block;");
        ...
        objDialogyGSTPC.dialog({ title: 'Add GST', width: '460', height: '400' });
        objDialogyGSTPC.dialog("open");
        return false;
    };
    ...
    self.ViewGSTTax = function (item) {
        ...
        objDialogyGSTPC.dialog({ title: 'View GST', width: '460', height: '400' });
        objDialogyGSTPC.dialog("open");
    };
    self.EditGSTTax = function (item) {
        ...
        $("#btnGSTTaxSubmit").css("display", "");
        objDialogyGSTPC.dialog({ title: 'GST Invoice Tax', width: '460', height: '400' });
        objDialogyGSTPC.dialog("open");
    };
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.Data1 != null && response.Data1 != "" && JSON.parse(response.Data1) != null) {
                    Data1 = JSON.parse(response.Data1);
                    if (Data1[0].Clear == false) {
                        objDialogyInvoicePC.dialog("close");
    ...
    this.LoadECFDetails = function () {
        ...
        $.ajax({
            type: "post",
            url: UrlDet + "GetEmployeePettyCash",
            data: JSON.stringify(EcfFilter),
            contentType: "application/json;",
            success: function (response) {
                $("#hfInvId").val("0");
...
ko.applyBindings(mainViewModel,document.getElementById("pettyCash"));
...
$(document).ready(function () {
    ...
    //Gopi
    objDialogyInvoicePC = $("[id$='divInvoicePettyCash']");
    objDialogyInvoicePC.dialog({
        autoOpen: false,
        modal: true,
        width: 1100,
        height: 650,
    ...
    //Gopi
    objDialogyGSTPC = $("[id$='GSTDialogPettyCash']");
    objDialogyGSTPC.dialog({
        autoOpen: false,
        modal: true,
        width: 460,
        height: 290,
