var objDialogyTemplate, objDialogyAddTemplate, objDialogyShipment, objDialogyAddShipment, objDialogyPODetails, objDialogyAddGst, objDialogyAddtaxdet;
...
var SearchModel = function (id) {
    ...
    self.POAddressArray = ko.observableArray();
    // add by gst
    //self.HsncodeArray = ko.observableArray();
    //self.TaxsubtypeArray = ko.observableArray();
    self.PoproviderArray = ko.observableArray();
    self.HSNArray = ko.observableArray();
    self.PoReciverArray = ko.observableArray();
    self.PoGsttatArray = ko.observableArray();
        ...
        if (POHeaderGId != "0") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.Data5 != null && response.Data5 != "" && JSON.parse(response.Data5) != null) {
                        ...
                        if (Data5.length > 0) {
                            for (var i = 0; i < Data5.length; i++) {
                                $('<option/>').val(Data5[i].ContactId).html(Data5[i].Addresslist).appendTo('#txtvendoraddress')
                            }
                    ...
                    $("#txtvendoraddress").val(Data2[0].VendorContact_gid);
                },
    ...
    this.SetPODetails = function () {
        //bharathidahsan kumar
        debugger;
        var _inBit = 0;
        var ProductGId = $("#ddlProductName").val();
        var OldproductGid = 0;
        var PODetailGId = $("#hfPODetailsGId").val();
        for (var m = 0; m < self.PODetailsArray().length; m++) {
            if (self.PODetailsArray()[m].PODetailsGId == PODetailGId) {
                _inBit = self.PODetailsArray()[m].ChkLstId;
                OldproductGid = self.PODetailsArray()[m].ProductGId;
                break;
            }
        }
        if (_inBit == 1 || OldproductGid != ProductGId) {
            jConfirm("Product Details Changed,Rework On Shipment & GST!", "Confirm", function (callback) {
                if (callback == true) {
                    var inputFilter = {
                        PODetailGId: PODetailGId
                    };
                    ...
                    $("#txtTaxes").val(0);
                    self.Productchange();
                    self.POShipmentArray.removeAll();
    ...
    this.SubmitPO = function () {
        ...
        var IsReject = "0";
        var Remarks = "";
        var VendorcontactId = $("#txtvendoraddress").val(); // add vendor cantact address
        ...
        if ($.trim(VendorcontactId) == "0") {
            jAlert("Ensure Vendor Address!", "Message");
            return false;
        }
        if ($.trim(POEndDate) == "") {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.Data1 != null && response.Data1 != "" && JSON.parse(response.Data1) != null) {
                    ...
                    if (Data1[0].Clear == true) {
                        hideProgress();
                        jAlert(Data1[0].Message + "Do you want continue to approve next claim", "Success", function (r) {
                            ...
                            if (r == true) {
                                ...
                                if (data2 == "0") {
                                 ...
                                 //   var dd = Object.keys(data2).length;

				    var dd = data2.length;

                                    for (var i = 0; i <= dd; i++) {
                                        var id = data2[i].queue_ref_gid;
                                        var name = data2[i].doc_status_gid;
    ...
    this.CheckedNotOk = function (obj, event) {
        ...
        var PODetailsGId = event.originalEvent.target.id;         
        var Header_type= 'D' ;
        ...
        if (_inBit == "1") {
            ...
            $("#htgstchk").val(0);
            jConfirm("Are you sure? Want to delete GST Tax Details!", "Confirm", function (callback) {
                if (callback == true) {
                    var inputFilter = {
                        POId: PODetailsGId,
    ...
    this.AddGstDetails = function (PODetailsGId, ChkLstId) {
        debugger;
        self.HSNArray([]);
        self.POShipmentArray([]);
        var _inBit = "";
        //  if (_inBit == "") {
        if (self.PODetailsArray().length > 0) {
            for (m = 0; m < self.PODetailsArray().length; m++) {
                if (self.PODetailsArray()[m].PODetailsGId == PODetailsGId) {
                    _inBit = 1 + '-' + PODetailsGId;
                    break;
                }
            }
        }
        ...
        if (_inBit == ChkLstId + '-' + PODetailsGId) {
            // if (poshipmentlist != 0) {

            $("#htgstchk").val(_inBit + '-' + PODetailsGId);
            var VendorId = $("#hfVendorName").val();
            if (VendorId != 0) {
                var inputFilter = {
                    PODetailGId: PODetailsGId,
                    VendorId: VendorId
                ...
                $.ajax({
                    ...
                    contentType: "application/json;",
                    success: function (response) {
                        var Data1 = "", Data2 = "", Data0 = "", Data3 = "", Data4 = "";
                        if (response.Data0 != null && response.Data0 != "" && JSON.parse(response.Data0) != null) {
                            Data0 = JSON.parse(response.Data0);
                            self.POShipmentArray(Data0);
                        }
                        ...
                        if (response.Data3 != null && response.Data3 != "" && JSON.parse(response.Data3) != null) {
                            //if (response.Data3.length > 51) {
                            Data3 = JSON.parse(response.Data3);
                            self.PoproviderArray(Data3);
                            if (response.Data2 != null && response.Data2 != "" && JSON.parse(response.Data2) != null) {
                                for (var j = 0; j < self.PoproviderArray().length; j++) {
                                    if (self.PoproviderArray()[j].Gsnno == Data2[0].Gsnno) {
                                        $("#txtgstproloc").val(Data3[j].Id);
                                        $("#txtgstinvendor").val(Data3[j].Gsnno);
                                        valueChange($("#txtgstproloc"));
                                        valueChange($("#txtgstinvendor"));
                                        break;
                                    }
                        ...
                        if (response.Data4 != null && response.Data4 != "" && JSON.parse(response.Data4) != null) {
                            ...
                            self.HSNArray(Data4);
                            if (response.Data4 != null && response.Data4 != "" && JSON.parse(response.Data4) != null) {
                                for (var j = 0; j < self.HSNArray().length; j++) {

                                    $("#txthsncode").val(Data4[j].hsn_gid);
                                    //$("#txthsncode").text(Data4[j].hsn_code);
                        ...
                        var poshipmentlist = self.POShipmentArray().length;
                        if (poshipmentlist != 0) {
                            $("#txtgstPORefNo").val($("#txtPORefNo").val());
                            $("#txtgstPODate").val($("#txtPODate").val());
                            $("#txtgstvename").val($("#txtVendorName").val());
                            $("#txtgstvencode").val($("#txtVendorCode").val());
                            $("#txtgstPOAmt").val($("#lblTotalAmount").text());
                            $("#hfPODetailsGId").val(PODetailsGId);
                            for (var i = 0; i < self.POShipmentArray().length; i++) {
                                if (self.POShipmentArray()[i].PODetailsGId == PODetailsGId) {
                                    var stategid = self.POShipmentArray()[i].Stategid;
                                    for (var j = 0; j < self.PoReciverArray().length; j++) {
                                        if (self.PoReciverArray()[j].Id == stategid) {
                                            $("#txtgstficcl").val(self.PoReciverArray()[j].BranchGstno);
                                            $("#txtgsrecloc").val(self.PoReciverArray()[j].Combo);
                                            $("#TxtGstReciverLocionId").val(self.PoReciverArray()[j].Id);
                            ...
                            if (response.Data1 != null && response.Data1 != "" && JSON.parse(response.Data1) != null)
                                Data1 = JSON.parse(response.Data1);
                            //var totalnetamount = Data1[0].Taxes + Data1[0].Total;
                            var totalnetamount = Data1[0].Total;
                           ...
                           // $("#txtgstnetamt").val((Data1[0].Total));
                            $("#txtgstnetamt").val(totalnetamount);
                            selectChange($("#txtgstprogrp"));
                            selectChange($("#txtgstprocode"));
                            selectChange($("#txtgstProname"));
                            //selectChange($("#txtgsttaxes"));
                            selectChange($("#txtgstuom"));
                            selectChange($("#txtgstqty"));
                            selectChange($("#txtgstrate"));
                            selectChange($("#txtgstbtamt"));
                            selectChange($("#txtgstnetamt"));

                            valueChange($("#txtgstPORefNo"));
                            valueChange($("#txtgstPODate"));
                            valueChange($("#txtgstvename"));
                            valueChange($("#txtgstvencode"));
                            valueChange($("#txtgstprogrp"));
                            valueChange($("#txtgstprocode"));
                            valueChange($("#txtgstProname"));
                            //valueChange($("#txtgsttaxes"));
                            valueChange($("#txtgstuom"));
                            valueChange($("#txtgstqty"));
                            valueChange($("#txtgstrate"));
                            valueChange($("#txtgstbtamt"));
                            valueChange($("#txtgstnetamt"));
                            valueChange($("#txtgstPOAmt"));

                            $('#Dialogaddgst').attr("style", "display:block;");
                            objDialogyAddGst.dialog({ title: 'Add GST Details', width: '1000', height: '600' });
                            objDialogyAddGst.dialog("open");
                            return false;
                        } else {
                            jAlert("Please Add Shipment Details!", "Message");
                        }
    ...
    this.TaxableChanged = function (obj, event)
    {
        //debugger;
        //var PODetailsGId = event.originalEvent.target.id;
        //var idval = $("#idvalues").val();
        ...
        //    Netval += parseFloat(total) + parseFloat(baseamt);
        //    $("#txtgstnetamt").val(Netval.toFixed(2));
        //    $("#txtgsttaxes").val(total.toFixed(2));
        //    var netamt = parseFloat($("#txtgstnetamt").val());
        //    var Totamt = parseFloat($("#lblTotalAmount").text());
        //    if (netamt > Totamt) {
    ...
    this.AddTaxdet = function () {
        $("#TxtTaxableamt").val($("#txtgstbtamt").val());
        $("#Txthsnid").val("0");
        $("#Txthsndesc").val("");
        $("#Txtsubtype").val("0");
        $("#Txttaxamt").val("");
        $("#TxttaxableRate").val("");
        selectChange($("#TxtTaxableamt"));
        selectChange($("#Txthsnid"));
        selectChange($("#Txthsndesc"));
        selectChange($("#Txtsubtype"));
        selectChange($("#Txttaxamt"));
        selectChange($("#TxttaxableRate"));

        $('#pogsttaxdel').attr("style", "display:block;");
        objDialogyAddtaxdet.dialog({ title: 'Add Tax Details', width: '600', height: '400' });
        objDialogyAddtaxdet.dialog("open");
        return false;
    };
    ...
    this.changeprovider = function () {
        debugger;
            for (var i = 0; i < self.PoproviderArray().length; i++) {
                if (self.PoproviderArray()[i].Id == $("#txtgstproloc").val()) {
                    $("#txtgstinvendor").val(self.PoproviderArray()[i].Gsnno);
                    var ProvLocId = $("#txtgstproloc").val();
                    var POId = $("#hfPODetailsGId").val();
                    var Basamt = $("#txtgstbtamt").val();
                    var ReciverLocId = $("#TxtGstReciverLocionId").val();
                    var Hsn_gid = $("#txthsncode").val();

               /*  var inputFilter = {
                        PODetailGId: POId,
                        ProvLocId: ProvLocId,
                        Baseamt: Basamt,
                        ReciverLocId: ReciverLocId
                    };
                    $.ajax({
                        type: "post",
                        url: UrlDet + "GetTaxdetails",
                        data: JSON.stringify(inputFilter),
                        async: false,
                        contentType: "application/json;",
                        success: function (response) {
    ...
    //Dhasarathan
    this.LoadGstCbodetls = function () {
        $.ajax({
            type: "post",
            url: UrlHelper + "Getgstcbodetls",
            data: "{}",
            async: false,
            contentType: "application/json;",
            success: function (response) {
                ...
                if (response.Data1 != null && response.Data1 != "" && JSON.parse(response.Data1) != null)
                    ...
                    //self.TaxsubtypeArray(Data2);
                    //self.PoproviderArray(Data3);
                    self.PoReciverArray(Data4);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                //alert(errorThrown);
            }
    ...
    this.SetPOGstDetails = function () {
        ...
        if (self.PoGsttatArray().length==0) {
            jAlert("Tax Details should not be Empty!", "Message");
            return false;
        }
        var TaxableAmount = 0;
        for (var i = 0; i < self.PoGsttatArray().length; i++) {
            TaxsubtypeId = TaxsubtypeId + self.PoGsttatArray()[i].TaxsubTypegid + "|";
            Taxablamt = Taxablamt + self.PoGsttatArray()[i].Taxableamt + "|";
            TaxableAmount += self.PoGsttatArray()[i].Taxableamt;
            TaxAmt = TaxAmt + self.PoGsttatArray()[i].Taxamt + "|";
            gstvalue +=  self.PoGsttatArray()[i].Taxamt;
        }
        debugger;
        var POHeaderGId = $("#hfPO").val();
        var PODetilsId = $("#hfPODetailsGId").val();
        var HsnId = self.PoGsttatArray()[0].hsn_gid;
        var TaxsubTypeId = TaxsubtypeId;
        var TaxablAmt = Taxablamt;
        var Taxmat = TaxAmt;
        var TaxRate = self.PoGsttatArray()[0].Rate;
        var grossamount = Number(netamt); //+ Number(gstvalue);
        //var overallamount = TaxableAmount + gstvalue;
        $("#lblTotalAmount").text(grossamount);
     //   TaxRate = TaxRate.replace("%","");
        ...
        $.ajax({
            type: "post",
            url: UrlDet + "SetPoGstDet",
            data: JSON.stringify(PoGstInsertdetls),
            contentType: "application/json;",
            success: function (response) {
                if (response.Data1 != null && response.Data1 != "" && JSON.parse(response.Data1) != null) {
                    Data1 = JSON.parse(response.Data1);
                    jAlert(Data1[0].Msg, "Message");
                    objDialogyAddGst.dialog("close");
                    self.GetPodetails();

                    //var overallamount = TaxableAmount + gstvalue;
                    $("#lblTotalAmount").text(Data1[0].OverallTotal);
                }
    ...
    this.SetAddGstDetails = function () {
        debugger;
        // for (var i = 0; i < self.PoproviderArray().length; i++) {
        //if (self.PoproviderArray()[i].Id == $("#txtgstproloc").val()) {
        //  $("#txtgstinvendor").val(self.PoproviderArray()[i].Gsnno);
        ...
        if (Hsn_gid == 0 || Hsn_gid=='undefined') {
            jAlert(" HSN Details should not be Empty!", "Message");
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                {
                    Totaltaxamount += Data2[i].Taxamt;
                }
                Totalnetamount = Number(Basamt)+ Number(Totaltaxamount);
                 $("#txtgstnetamt").val(Totalnetamount);
    ...
    self.LoadPOAttachment();
    //add gst
    self.LoadGstCbodetls();



    $("#txtVendorName").keyup(function (e) {
        debugger;
        var IsExists = $('#txtVendorCode').val();
        if (e.which != 13) {
            $("#hfVendorName").val("0");
...
$(document).ready(function () {
    ...
    objDialogyAddGst = $("[id$='Dialogaddgst']");
    objDialogyAddGst.dialog({
        autoOpen: false,
        modal: true,
        width: 800,
        height: 500,
    ...
    //objDialogyAddtaxdet = $("[id$='pogsttaxdel']");
    //objDialogyAddtaxdet.dialog({
    //    autoOpen: false,
    //    modal: true,
    //    width: 800,
    //    height: 400,
...
function CloaseDialog(flag) {
    if (flag == "0")
        ...
        objDialogyAddtaxdet.dialog("close");
    return false;
}
