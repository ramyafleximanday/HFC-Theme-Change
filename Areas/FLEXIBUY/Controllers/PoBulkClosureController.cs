...
{
    public class PoBulkClosureController : Controller
    {
        //
        ...
        int count = 0;
        public PoBulkClosureController()
            : this(new fb_DataModelpr())
        {

        }
        public PoBulkClosureController(Irepositorypr objM)
        {
            objModel = objM;
        }
        public ActionResult PoBulkClosure(string viewfor)
        {
            PoBulkClosure objClosure = new PoBulkClosure();
            ...
            if (viewfor == "cancel")
            {
                ...
                List<PoBulkClosure> lLstClosure = new List<PoBulkClosure>();
                objClosure.lListBulkUpload = lLstClosure;
                if (objClosure.lListBulkUpload.Count == 0)
                {
                    ViewBag.records = "No Records Found";
                }
                return View("PoBulkClosure",objClosure);
            }
            if (Session["maindatatable"] != null)
            {
                DataTable dt = (DataTable)Session["maindatatable"];
                List<PoBulkClosure> objlist = new List<PoBulkClosure>();
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    objClosure = new PoBulkClosure();
                    objClosure.slNo = i + 1;
                    objClosure.poNo = dt.Rows[i]["PONo"].ToString();
                    objClosure.poDate = obj.ddMMyyyyString(dt.Rows[i]["PoDate"].ToString());
        ...
        public async Task<JsonResult> FileUpload()
        {
            ...
            try
            {
                ...
                if (Session["closurefilename"] != null)
                {
                    ...
                    string n = string.Format("PoBulkClosure-{0:yyyy-MM-dd_hh-mm-ss-tt}", DateTime.Now);
        ...
        public ActionResult DownloadExcel()
        {
            ...
            Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "PoBulkClosure.xls"));
        ...
        public ActionResult DownloadErrorLogExcel()
        {
            ...
            Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "PoBulkClosureErrlog.xls"));
            Response.ContentType = "application/vnd.ms-excel";
            //Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            DataTable dt = dtnew;
            string str = string.Empty;
        ...
        public JsonResult ColumnValidation(BulkUpload objValid)
        {
            try
            {
                ...
                //    Session["ExcelTable"] = dataTable;
                    ...
                    if (strColArr[0].ToString() == "SNo" && strColArr[1].ToString() == "PONo" && strColArr[2].ToString() == "PoDate" && strColArr[3].ToString() == "Remarks" && strColArr.Count().ToString() == "4")
                    {
                        result = "Success";
                    }
                //}
        ...
        [HttpGet]
        public PartialViewResult BulkUploadStatusForPo()
        {
            PoBulkClosure objValid = new PoBulkClosure();
            try
            {
            ...
            try
            {
                if (objTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < objTable.Rows.Count; i++)
                    {
                        ...
                        {
                            errs = "PoNo Should Not be Empty ";
                            columnName = objTable.Columns[1].ToString();
                        }
                        else
                        {
                            exceltext = objTable.Rows[i][1].ToString();
                            status = objModel.GetPoClosureExcelValid(exceltext,"", "pono");
                            if (status == "notexists")
                            {
                                if (errs == "" && columnName == "")
                                {
                                    errs = "PoNo Was Not Found ";
                                    columnName = objTable.Columns[1].ToString();
                                }
                                else
                                {
                                    errs = errs + "," + "PoNo Was Not Found ";
                        ...
                        if (objTable.Rows[i][2].ToString() == "")
                        {
                            ...
                            if (errs == "" && columnName == "")
                            {
                                errs = "PoDate Should Not be Empty";
                                columnName = objTable.Columns[2].ToString();
                            }
                            else
                            {
                                errs = errs + "," + "PoDate Should Not be Empty";
                        ...
                        else
                        {
                            ...
                            //}
                                ...
                                status = objModel.GetPoClosureExcelValid(exceltext,objTable.Rows[i][1].ToString(), "podate");
                                if (status == "notexists")
                                {
                                    if (errs == "" && columnName == "")
                                    {
                                        errs = "PoDate Was Not Found ";
                                        columnName = objTable.Columns[2].ToString();
                                    }
                                    else
                                    {
                                        errs = errs + "," + "PoDate Was Not Found ";
                                        columnName = columnName + "," + objTable.Columns[2].ToString();
                                    }
                                }
        ...
        public JsonResult InsertBulkClosure()
        {
            try
            {
                ...
                if (Session["maindatatable"] != null && Session["validrecords"] != null)
                {
                    ...
                    string fileName = "";
                    objClosure.result = objModel.BulkPoInsert(dt, validRecords, fileName);
                }
